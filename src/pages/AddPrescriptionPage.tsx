...
const AddPrescriptionPage = () => {
  ...
  useEffect(() => {
    if (isEditMode && id) fetchExistingPrescription();
  ...
  const fetchExistingPrescription = async () => {
    ...
    try {
      ...
      toast.error('فشل في تحميل بيانات الروشتة');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !doctorName.trim() || !specialty) return toast.error('يرجى ملء كافة البيانات الأساسية');
    ...
    try {
      ...
      if (isEditMode && id) {
        ...
        await updatePrescription(Number(id), formData);
  ...
  if (isLoadingData) return <div className="h-screen flex items-center justify-center font-cairo"><LoadingSpinner /></div>;

  return (
      <div className="min-h-screen bg-background pb-10 font-cairo text-right" dir="rtl">
        <PageHeader title={isEditMode ? "تعديل روشتة" : "إضافة روشتة جديدة"} />
        <form onSubmit={handleSubmit} className="px-4 py-4 space-y-6 max-w-2xl mx-auto">
          ...
          <div className="space-y-4 bg-card p-5 rounded-2xl border border-border shadow-sm">
            <div className="space-y-2">
              <Label className="font-bold text-primary text-sm">عنوان الروشتة</Label>
              <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="مثلاً: روشتة عيادة الباطنة" className="rounded-xl" />
            </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="min-h-[100px] rounded-xl" placeholder="دواء الضغط مرتين يومياً..." />
          </div>
          ...
          {/* أزرار الكاميرا والمعرض الاحترافية */}
          <div className="space-y-4">
            <Label className="font-bold text-primary text-sm">إضافة صور الروشتة</Label>
            <div className="grid grid-cols-2 gap-4">
              <button type="button" onClick={() => triggerPicker('image/*', 'environment')} className="flex flex-col items-center gap-3 p-6 bg-blue-50 border-2 border-dashed border-blue-200 rounded-2xl hover:bg-blue-100 active:scale-95 transition-all shadow-sm">
                <Camera className="w-8 h-8 text-blue-600" />
                <span className="text-xs font-black text-blue-700">كاميرا</span>
              </button>
              <button type="button" onClick={() => triggerPicker('image/*')} className="flex flex-col items-center gap-3 p-6 bg-emerald-50 border-2 border-dashed border-emerald-200 rounded-2xl hover:bg-emerald-100 active:scale-95 transition-all shadow-sm">
          ...
          <div className="flex gap-4 pt-4">
            <Button
                ...
                variant="ghost"
                onClick={() => navigate(-1)}
                className="flex-1 h-12 font-black rounded-xl border-2 border-slate-100 text-slate-500"
            >
              إلغاء
            </Button>
            ...
            <Button
                ...
                className="flex-1 h-12 gap-2 shadow-lg font-black rounded-xl bg-primary hover:bg-primary/90 text-white shadow-primary/20"
            >
              {isSubmitting ? (
